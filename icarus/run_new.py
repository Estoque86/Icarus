...
from icarus.scenario_new import scenario_generator, req_generator

def run_simulation_scenario(t, a, c, s, seed):
    ...
    scenario_id = 'SIM=ICARUS_T=%s_REQ=%s_M=%s_C=%s_L=%s_A=%s_R=%s' % (t, str(requests), str(n_contents), str(c), str(rate), str(a), seed)
    topo = read_topology(topo_file)
    if use_events_file:
        es_file = path.join(scenarios_dir, es_prefix + 'T=%s@A=%s.xml' % (t, str(a)))
        es = read_event_schedule(es_file)
    else:
        es = req_generator(topo, n_contents, a, seed)
    ...
    for t in topologies:
        for a in alpha:
            for c in net_cache:
                for s in strategies:
                    if parallel_exec:
                        last_job = pool.apply_async(run_simulation_scenario, args=(t, a, c, s, seed))
                    else:
                        run_simulation_scenario(t, a, c, s, seed)
