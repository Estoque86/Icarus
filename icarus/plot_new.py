...
ALPHA_RANGE = arange(0.6, 1.4, 0.2)
...
def plot_chr_alpha_sensitivity(show=False, save=True):
    """
    ...
    for t in T:
        for c in [str(nc) for nc in C]:
            ...
            plt.xlabel('Content distribution ')
            ...
            if show: plt.show()
            if save: plt.savefig(path.join(GRAPHS_DIR , 'CACHE_HIT_T=%s@C=%s.png' % (t, c)), bbox_inches='tight')
